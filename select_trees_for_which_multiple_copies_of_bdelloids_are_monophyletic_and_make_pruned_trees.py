...
#target group of species have multiple copies (e.g. clade Bdelloidea within Rotifera); Check that sequences 
#of target clade are monophyletic. Prune trees to keep only sequences of target clade (with multiple copies) and their corresponding fasta files 
...
def main():
	...
	for treefile in treefiles:
		...
		if t.check_monophyly(values=[k for k in heads_seqs_target.keys()], target_attr="name")[1]=='monophyletic':
			...
			#don't print any support values
			t.write(outfile=outpath+OG_ID+".treefile", format=5)
